...
when ODIN_ARCH == .wasm32 || ODIN_ARCH == .wasm64p32 {
    // For WebAssembly, expect 'lib/cgltf_wasm.o'
    // This file needs to be compiled using emcc (e.g., from cgltf.c) and placed in the 'vendor/cgltf/lib' directory.
    // It must also be linked by emcc in the final WASM build step (see build_game.odin).
    when !#exists("lib/cgltf_wasm.o") {
        #panic("cgltf: Could not find 'lib/cgltf_wasm.o'. This precompiled WASM object file is required. Ensure it is present in 'vendor/cgltf/lib' and that cgltf.c has been compiled for WASM.")
    }
	foreign import lib "lib/cgltf_wasm.o"
} else when ODIN_OS == .Windows {
    // For Windows, expect 'lib/cgltf.lib'
    when !#exists("lib/cgltf.lib") {
        #panic("cgltf: Could not find 'lib/cgltf.lib'. This library file is required. It can typically be compiled by running `make -C \"" + ODIN_ROOT + "vendor/cgltf/src\"` or ensuring it is present in 'vendor/cgltf/lib'.")
    }
	foreign import lib "lib/cgltf.lib"
} else when ODIN_OS == .Linux {
    // For Linux, expect 'lib/cgltf.a'
    when !#exists("lib/cgltf.a") {
        #panic("cgltf: Could not find 'lib/cgltf.a'. This library file is required. It can typically be compiled by running `make -C \"" + ODIN_ROOT + "vendor/cgltf/src\"` or ensuring it is present in 'vendor/cgltf/lib'.")
    }
	foreign import lib "lib/cgltf.a"
} else when ODIN_OS == .Darwin {
    // For Darwin (macOS), expect 'lib/darwin/cgltf.a'
    when !#exists("lib/darwin/cgltf.a") {
        #panic("cgltf: Could not find 'lib/darwin/cgltf.a'. This library file is required. It can typically be compiled by running `make -C \"" + ODIN_ROOT + "vendor/cgltf/src\"` or ensuring it is present in 'vendor/cgltf/lib/darwin'.")
    }
	foreign import lib "lib/darwin/cgltf.a"
} else {
    // Fallback for other OSes or if no specific library path is defined for the host.
    // Attempts to link against a system-provided 'cgltf' library.
